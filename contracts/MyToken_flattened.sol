 ...
 */
abstract contract ERC20 is IERC20, IERC20Metadata, IERC20Errors, Ownable2Step {
    mapping(address account => uint256) private _balances;

    mapping(address account => mapping(address spender => uint256)) private _allowances;

    mapping(uint8 key=> typeOf ) public _typeOfs;
    mapping(address account => lockOf) public _lockOfs;
    ...
    */
    struct lockOf {
        uint32 _time;    //lock at timestamp
        uint8 _lock;     //how month 1 - 255 month
        uint8 _rate;     //rate 20 or 25
        uint64 _locked;  //lock balance
    }
    ...
    function _update(address from, address to, uint256 value) internal virtual {
        if (from == address(0)) {
            ...
            uint _locked = 0;
            lockOf memory _lock = _lockOfs[from];
            if (_lock._locked > 0) { 
                _locked = locked(from);
                if(_locked <= 0) {
                    delete _lockOfs[from];
                }
            }
            if (fromBalance - _locked < value) {
    ...
    /**
     * @dev return lock number
     */
    function locked(address account) public view virtual returns (uint256) {
        lockOf memory _lock = _lockOfs[account];
        if (_lock._locked > 0) {
            uint _long = uint(block.timestamp) - uint(_lock._time);
            uint _locked = _lock._locked * (_long / (24*3600*30*uint(_lock._lock))) * uint(_lock._rate) / 100;
            uint _last = 0;
            if ( _locked > _lock._locked ) {
                return _last;
            } else {
                return _lock._locked - _locked;
            }
        }
        return 0;
    }
...
contract MyToken is ERC20, ERC20Permit {
    ...
    constructor() ERC20("Btc Football", "BFB") ERC20Permit("Btc Football") Ownable(_msgSender()) {
